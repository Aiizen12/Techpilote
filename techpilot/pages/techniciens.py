...
class TechniciensState(rx.State):
    ...
    def open_edit(self, tech: dict):
        self.edit_id = str(tech.get("id",""))
        self.form = {**tech}
        self.show_form = True
...
def tech_card(tech: dict) -> rx.Component:
    color = tech.get("color") or PRIMARY
    nom = tech.get("nom") or tech.get("name") or "?"
    return rx.box(
        rx.vstack(
            rx.hstack(
                rx.box(rx.text(nom[:2].upper(), color="white", font_weight="700"), background=color, border_radius="50%", width="42px", height="42px", display="flex", align_items="center", justify_content="center"),
                rx.spacer(),
                rx.cond(tech.get("active",True), rx.badge("Actif", color_scheme="green", variant="soft", radius="full"), rx.badge("Inactif", color_scheme="gray", variant="soft", radius="full")),
            ),
            rx.text(nom, color=TEXT, font_weight="600", font_size="1rem"),
            rx.text(f"#{tech.get('matricule','')}", color=MUTED, font_size="0.8rem"),
            rx.text(tech.get("email",""), color=MUTED, font_size="0.78rem"),
            rx.hstack(
                rx.button("Modifier", on_click=TechniciensState.open_edit(tech), background="rgba(99,102,241,0.1)", color=PRIMARY, border=f"1px solid rgba(99,102,241,0.3)", border_radius="6px", padding="5px 12px", font_size="0.78rem", cursor="pointer"),
                rx.button(
                    rx.cond(tech.get("active",True), "Désactiver", "Activer"),
                    on_click=TechniciensState.toggle_active(str(tech.get("id",""))),
                    background="transparent", color=MUTED, border=f"1px solid {BORDER}", border_radius="6px", padding="5px 12px", font_size="0.78rem", cursor="pointer",
                ),
                spacing="2",
            ),
        ...
        _hover={"border_color": color}, transition="border-color 0.2s",
    )
    ...
    return rx.vstack(
        ...
        rx.dialog.root(
            rx.dialog.content(
                ...
                rx.vstack(
                    rx.input(placeholder="Nom *", value=TechniciensState.form.get("nom",""), on_change=lambda v: TechniciensState.set_field("nom",v), background="#1e2035", color=TEXT, border=f"1px solid {BORDER}", border_radius="8px", width="100%"),
                    rx.input(placeholder="Matricule", value=TechniciensState.form.get("matricule",""), on_change=lambda v: TechniciensState.set_field("matricule",v), background="#1e2035", color=TEXT, border=f"1px solid {BORDER}", border_radius="8px", width="100%"),
                    rx.input(placeholder="Email", value=TechniciensState.form.get("email",""), on_change=lambda v: TechniciensState.set_field("email",v), background="#1e2035", color=TEXT, border=f"1px solid {BORDER}", border_radius="8px", width="100%"),
                    rx.hstack(
